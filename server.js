...
app.get("/api/currentStocks", (req, res) => {
  const query = `WITH StocksBought AS (
            ...
            StocksCurrent AS (
                ...
                FROM StocksBought b
                LEFT JOIN StocksSold s ON b.id_company = s.id_company
                UNION
                SELECT
                    s.id_company,
                    COALESCE(b.total_bought, 0) - COALESCE(s.total_sold, 0) AS current_stock
...
app.get("/api/transactions/:type/:id_company", (req, res) => {
  ...
  if (!idCompany) {
    return res.status(400).json({
      error: "id_company route parameter is required: .../:id_company",
    });
  }
...
// Gets all buy/sell transactions
